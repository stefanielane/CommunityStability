...
newnames99 = c("Agrostis_stolonifera","Symphyotrichum_subspicatum", "Mentha_aquatica", "Schoenoplectus_tabernaemontani", "Trifolium_wormskioldii")
...
newnames79 = c("Agrostis_stolonifera","Symphyotrichum_subspicatum", "Galium_trifidum", "Hypericum_scouleri", "Mentha_aquatica", "Puccinellia_pauciflora", "Schoenoplectus_tabernaemontani", "Trifolium_wormskioldii")
...
dend2019 <-  as.dendrogram(d2019_ward) %>%
   set("branches_lwd", 1.25) %>% # Branches line width
   set("branches_k_color", mycols2019, k = 3) %>% # Color branches by groups
   set("labels_col", "black") %>%   # mask labels by changing color
...
dend1979 <-  as.dendrogram(d1979_ward) %>%
   ...
   set("branches_k_color", 
       mycols1979, 
       k = 3) %>% # Color branches by groups
    set("labels_col", 
        "black") %>%  # mask labels by changing color
   #set("labels_colors", mycols, k = 3) %>%  # Color labels by groups
   set("labels_cex", 0.5) # Change label size
#fviz_dend(dend2) 
...
# another good tutorial at https://sites.ualberta.ca/~ahamann/teaching/renr690/labs/Lab9h.pdf
...
d1979_clstrs <- align79 %>% mutate(Association_Type2 = case_when(Quadrat %in% c(30, 31, 27, 24, 28, 29, 32, 108, 25, 26, 60, 48, 57, 58, 59, 49, 68, 69, 22, 23, 86, 93, 87, 88, 97, 94, 95, 96, 20, 72, 47, 73, 55, 56) ~1, 
                                                                 Quadrat %in% c(80, 81, 64, 62, 65, 82, 83, 98, 84, 85, 78, 79, 63, 66, 50, 99, 100, 61, 67) ~3, 
                                                                 Quadrat %in% c(38, 74, 11, 12, 107, 9, 10, 76, 106, 102, 101, 103, 42, 43, 14, 15, 40, 41, 39, 44, 21, 37, 16, 13, 104, 105, 75, 46, 77) ~2))
#view(d1979_clstrs)
...
```{r EXTRA - ISA with different cluster groups (ED = 35)}

###########################
# 2019
###########################

d2019_2clstrs <- align19 %>% mutate(Association_Type2 = case_when(Quadrat %in% c(26, 72, 23, 22, 5, 24, 39, 73, 20, 21, 25, 6, 7, 36, 37, 35, 38, 4, 34, 33, 71, 63, 11, 62, 48, 49, 27, 42) ~1, Quadrat %in% c(44, 43, 47, 12, 32, 1, 66, 17, 29, 46, 31, 69, 28, 65, 67, 68, 30, 59, 19, 40, 74, 8, 10, 13, 9, 54, 50, 15, 55, 41, 70, 2, 52, 57, 53, 56, 64, 3, 16, 51, 58, 14, 18, 45, 60, 61) ~2))
#View(d2019_clstrs)
#Note: new column must be Association_Type2 to join with other data sets for NMS

#move new column to beginning of df (if left at end, new column Association_Type gets counted in the indicspecies)
d2019_2grps <- d2019_2clstrs %>% 
  select(Association_Type2, everything())


abund2 = d2019_2grps[,5:ncol(d2019_2grps)]
assoc2 = d2019_2grps$Association_Type2

## turn comments off to run - takes a minute
inv_r.g2 = multipatt(abund2, assoc2, func = "r.g", control = how(nperm=9999))
summary(inv_r.g2, indvalcomp = TRUE)
...
d1999_2clstrs <- align99 %>% mutate(Association_Type2 = case_when(Quadrat %in% c(39, 40, 31, 29, 30, 27, 26, 28, 32, 69, 22, 46, 23, 24, 87, 88, 94, 73, 72, 108, 85, 42, 44, 41, 43, 96, 95, 97, 12, 100, 101) ~1, Quadrat %in% c(82, 83, 13, 14, 84, 77, 78, 102, 103, 105, 98, 99, 81, 80, 104, 16, 15, 79, 10, 11, 37, 38, 20, 93, 107, 9, 106, 62, 25, 59, 61, 56, 57, 58, 68, 60, 67, 55, 86, 65, 66, 63, 64, 76, 74, 75, 50, 47, 49, 21, 48) ~2))

#view(d1999_clstrs)

#move new column to beginning of df (if left at end, new column Association_Type gets counted in the indicspecies)
d1999_2grps <- d1999_2clstrs %>% 
  select(Association_Type2, everything())
#view(d1999_grps)

abund99_2 = d1999_2grps[,5:ncol(d1999_2grps)]
assoc99_2 = d1999_2grps$Association_Type2

## turn comments off to run - takes a minute
inv99_r.g2 = multipatt(abund99_2, assoc99_2, func = "r.g", control = how(nperm=9999))
summary(inv99_r.g2)
...
```{r - how many plots per transect fall within each assemblage type?}
...
gp_all <- rbind(gp79, gp99, gp19) 

spat1 <- gp_all %>% 
              mutate(Association_Type2 = recode(Association_Type2, 
                                                "1" = "sedge", 
                                                "2" = "fescue",
                                                "3" = "bogbean")) %>% 
...
spat2 <- spat1 %>% 
            group_by(Year, Transect, Association_Type2) %>% 
            mutate(n_assmb = n(), 
                   pct = round(100*n_assmb/n_xsect, 0))
...
spatial$Year <- as.factor(spatial$Year)

spatial$Association_Type2 <- factor(spatial$Association_Type2, 
                                    levels = c("sedge", "fescue", "bogbean"), 
...
spatial_bar <- spatial %>% 
            ...
            geom_col(position = position_dodge(width = 1, 
                                               preserve = "single")) +
            theme_classic() + 
            labs(y = "Percentage of plots per transect in each cluster assemblage",
            ...
            facet_grid(Transect ~ Association_Type2, 
                       space = "free") 
spatial_bar

#ggsave("fig-spatial-bar.jpg", spatial_bar, width = 12, height = 12)


```

```{r Supplemental - ISA of sixth highest-level subgroups}

subgp79 <- align79 %>% mutate(Association_Type2 = 
                                case_when(Quadrat %in% c(30, 31, 27, 24, 28, 29, 32, 108, 25, 26) ~1.1, 
                                          Quadrat %in% c(60, 48, 57, 58, 59, 49, 68, 69, 22, 23) ~1.2, 
                                          Quadrat %in% c(86, 93, 87, 88, 97, 94, 95, 96, 20, 72, 47, 73, 55, 56) ~1.3, 
                                          Quadrat %in% c(80, 81, 64, 62, 65, 82, 83, 98, 84, 85, 78, 79, 63, 66, 50, 99, 100, 61, 67) ~3,
                                          Quadrat %in% c(38, 74, 11, 12, 107, 9, 10, 76, 106) ~2.1,
                                          Quadrat %in% c(102, 101, 103, 42, 43, 14, 15, 40, 41, 39, 44, 21, 37, 16, 13, 104, 105, 75, 46, 77) ~2.2))
#View(subgp79)
sub79 <- subgp79 %>% 
  select(Association_Type2, everything())
#View(sub79)
...
abund79sub = sub79[,5:ncol(sub79)]
assoc79sub = sub79$Association_Type2

## turn comments off to run - takes a minute
inv_79gp = multipatt(abund79sub, 
                     assoc79sub, 
                     func = "r.g", 
                     control = how(nperm=999))

summary(inv_79gp, indvalcomp = TRUE, alpha = 1)

#~#~#~#~#~#~#~#~#~#~#
## 1999
#~#~#~#~#~#~#~#~#~#~#

subgp99 <- align99 %>% mutate(Association_Type2 = 
                                case_when(Quadrat %in% c(39, 40, 31, 29, 30, 27, 26, 28, 32, 69, 22, 46, 23, 24, 87, 88, 94, 73, 72, 108) ~1.1, 
                                          Quadrat %in% c(85, 42, 44, 41, 43, 96, 95, 97, 12, 100, 101) ~1.2, 
                                          Quadrat %in% c(82, 83, 13, 14, 84, 77, 78, 102, 103, 105, 98, 99, 81, 80, 104, 16, 15, 79) ~3,
                                          Quadrat %in% c(10, 11, 37, 38, 20, 93, 107, 9, 106) ~2.1,
                                          Quadrat %in% c(62, 25, 59, 61, 56, 57, 58, 68, 60, 67, 55, 86) ~2.2, 
                                          Quadrat %in% c(65, 66, 63, 64, 76, 74, 75, 50, 47, 49, 21, 48) ~2.3))
#View(subgp79)
sub99 <- subgp99 %>% 
  select(Association_Type2, everything())
#View(sub79)
...
abund99sub = sub99[,5:ncol(sub99)]
assoc99sub = sub99$Association_Type2

## turn comments off to run - takes a minute
inv_99gp = multipatt(abund99sub, 
                     assoc99sub, 
                     func = "r.g", 
                     control = how(nperm=999))

summary(inv_99gp, indvalcomp = TRUE, alpha = 1)
...
subgp19 <- align19 %>% mutate(Association_Type2 = 
                                case_when(Quadrat %in% c(19, 40, 74, 8, 10, 13, 9, 54) ~1.1, 
                                          Quadrat %in% c(50, 15, 55, 41, 70, 2, 52, 57, 53, 56, 64, 3, 16, 51, 58) ~1.2, 
                                          Quadrat %in% c(14, 18, 45, 60,61) ~1.3,
                                          Quadrat %in% c(38, 35, 37, 36, 7, 6, 25, 21, 20, 73, 39, 24, 5, 22, 23, 72, 26) ~3.1,
                                          Quadrat %in% c(4, 34, 33, 71, 63, 11, 62, 48, 49, 27, 42) ~3.2,
                                          Quadrat %in% c(44, 43, 47, 12, 32, 1, 66, 17, 29, 46, 31, 69, 28, 65, 67, 68, 30, 59) ~2))
#View(subgp79)
sub19 <- subgp19 %>% 
  select(Association_Type2, everything())
#View(sub79)
...
abund19sub = sub19[,5:ncol(sub19)]
assoc19sub = sub19$Association_Type2

## turn comments off to run - takes a minute
inv_19gp = multipatt(abund19sub, 
                     assoc19sub, 
                     func = "r.g", 
                     control = how(nperm=999))

summary(inv_19gp, indvalcomp = TRUE, alpha = 1)
...
rich$rich_status <- factor(rich$rich_status, 
                           ...
                           labels = c("Native", "Non-native", "Unknown"))
...
freq.bogbean <- join_40 %>% 
   ...
   mutate(Species = recode(Species, "Menyanthes_trifoliata" = "Menyanthes \ntrifoliata", "Mentha_aquatica" = "Mentha \naquatica*", "Lythrum_salicaria" = "Lythrum \nsalicaria*", "Sidalcea_hendersonii" = "Sidalcea \nhendersonii", "Phalaris_arundinacea" = "Phalaris \narundinacea*", "Iris_pseudacorus" = "Iris \npseudacorus*"))

# FEAR, Salix_lasiandra (tip for Disc), LYSA/SIHE, PHAR/Iris_pseudacorus 
#### NOTE: nomenclature for Festuca arundinacea (FEAR) updated to Schedonorus arundinaceus
freq.fescue <- join_40 %>% 
  ...
  mutate(Species = recode(Species, "Festuca_arundinacea" = "Schedonorus \narundinaceus*", "Salix_lasiandra" = "Salix \nlucida",  "Sidalcea_hendersonii" = "Sidalcea \nhendersonii", "Lythrum_salicaria" = "Lythrum \nsalicaria*", "Phalaris_arundinacea" = "Phalaris \narundinacea*", "Iris_pseudacorus" = "Iris \npseudacorus*")) 
...
freq.sedge <- join_40 %>% 
   ...
   mutate(Species = recode(Species, "Carex_lyngbyei" = "Carex \nlyngbyei", "Salix_lasiandra" = "Salix \nlucida",  "Sidalcea_hendersonii" = "Sidalcea \nhendersonii", "Lythrum_salicaria" = "Lythrum \nsalicaria*", "Phalaris_arundinacea" = "Phalaris \narundinacea*", "Iris_pseudacorus" = "Iris \npseudacorus*"))
...
freq.bar <- annotate_figure(freq.panel, 
                            left = text_grob("Cover abundance class (mean)", 
                                             size = 20, rot = 90, hjust = 0.25, vjust = 1), 

...
# View(clock_sp)
# x = species, color = year, y = cover class, panel = assemblage
...
freq.sp <- join_40 %>% 
   filter(Species %in% c("AGST2", "BICE", "CALY3", "DECE", "ELPA3", "EQFL", "EQPA", "EQVA", "FEAR3", 
                         "GATR3", "IMCA", "JUAR",  "LAPA4", "LYSA2", "LYTH2", "MEAQ", "METR3", "MYSC", 
                         "PHAR3", "POPA2", "SALA2", "SALUL", "SCTA2", "SIHE4", "TRWO"))  
freq.sp$Year <- as.factor(freq.sp$Year)

freq.bogbean <- freq.sp %>% 
   filter(Assemblage == "Bogbean") %>% 
   filter(Species %in% c("METR3", "MYSC", "MEAQ", "BICE", "LYSA2", "LYTH2")) %>% 
   mutate(Species = recode(Species, "METR3" = "Menyanthes trifoliata", "MYSC" = "Myosotis scorpiodes*", "MEAQ" = "Mentha aquatica*", "BICE" = "Bidens cernua", "LYSA2" = "Lythrum salicaria*", "LYTH2" = "Lysimachia thyrsiflora"))
...
# freq <- ggplot(freq.sp, aes(x = Species, y = mean_cover, fill = Year)) +
...
col.bogbean <- ggplot(freq.bogbean, aes(x = Species, y = mean_cover, fill = Year)) +
  geom_bar(stat="identity", position = position_dodge(width = 0.5)) + theme_classic() +
  labs(y = "", x = "") +
  theme(axis.text.x = element_text(size = 12, angle = 30, hjust = 1), strip.text = element_text(
    size = 12)) +
  scale_fill_manual(values = freq.col) +
  facet_grid(.~ Assemblage)
col.bogbean

col.sedge <- ggplot(freq.sedge, aes(x = Species, y = mean_cover, fill = Year)) +
  geom_bar(stat="identity", position = position_dodge(width = 0.5)) + theme_classic() +
  labs(y = "", x = "") +
  theme(axis.text.x = element_text(size = 12, angle = 30, hjust = 1), strip.text = element_text(
    size = 12)) +
...
col.fescue <- ggplot(freq.fescue, aes(x = Species, y = mean_cover, fill = Year)) +
  geom_bar(stat="identity", position = position_dodge(width = 0.5)) + theme_classic() +
  labs(y = "", x = "") +
  theme(axis.text.x = element_text(size = 12, angle = 30, hjust = 1), strip.text = element_text(
    size = 12)) +
...
## panel format w/ ggpubr
library(ggpubr)
freq.panel <- ggarrange(col.sedge, col.fescue, col.bogbean,
          ncol = 1, nrow = 3, align = "hv") 
freq.panel

freq.bar <- annotate_figure(freq.panel, left = text_grob("Cover abundance class (mean)", size = 20, rot = 90, hjust = .5, vjust = 1), bottom = text_grob("Significant Indicator Species", size = 20))
freq.bar

...
d1999 <- read.csv("1999_Denoth-NRCS.csv") # if first column has a byte order mark
#colnames(d1999)[1] <- gsub('^...','',colnames(d1999)[1])
#view(d1999)

d1979 <- read.csv("1979_Bradfield-NRCS.csv") # if first column has a byte order mark
...
join_40 <- lst(d1979_grps, d1999_grps, d2019_grps) %>% 
  reduce(full_join) %>% 
  replace(is.na(.), 0) %>% 
  pivot_longer(AGST2:SCMI2, names_to = "Species", values_to = "Cover_class") %>% 
...
sp_table <- join_40 %>%
  ...
  mutate(Status = ifelse(Species %in% c("AGST2", "CIAR4", "FEAR3", "IRPS", "LYEU", "LYSA2", "MEAQ", "MEAR", "MYSC", "PHAR3", "RUCO2"), "Non-native",
                ifelse(Species %in% c("Galium_sp", "Poaceae_misc", "Asteraceae_misc", "Carex_sp1", "Festuca_sp", "Salix_sp"), "Unknown", "Native"))) %>%
  mutate(Species = recode(Species, 
            "AGST2" = "Agrostis stolonifera", "ALGE2" = "Alopecurus geniculatus", "ALPL"  = "Alisma plantago aquatica",
            "BICE"  = "Bidens cernua", "CALY3" = "Carex lyngbyei", "CAPAP6" = "Caltha palustris", "DECE"  = "Deschampsia caespitosa",
            "ELPA3" = "Eleocharis palustris", "EQAR"  = "Equisetum arvense", "EQFL"  = "Equisetum fluviatile", "EQPA" = "Equisetum palustre",
            "EQVA" = "Equisetum variegatum", "FEAR3" = "Festuca arundinacea", "Galium_sp" = "Galium sp.", "GATR2"  ="Galium trifidum",
            "HYSC5"  = "Hypericum scouleri", "IMCA" = "Impatiens capensis", "IRPS"  = "Iris pseudocorus", "JUAC" = "Juncus acuminatus",
            "JUAR"  = "Juncus articulatus", "JUOX" = "Juncus oxymeris", "LAPA4" = "Lathyrus palustris", "LEOR"  = "Leersia oryzoides",
            "LIOC"  = "Lilaeopsis occidentalis","LYAM3" = "Lysichiton americanum","LYEU" = "Lycopus europaeus","LYSA2" =  "Lythrum salicaria",
            "LYTH2" = "Lysimachia thyrsiflora","MEAQ"  = "Mentha aquatica","MEAR4" = "Mentha arvensis","METR3" = "Menyanthes trifoliata",
            "MYSC"  = "Myosotis scorpiodes","OESA" = "Oenanthe sarmentosa","PHAR3" = "Phalaris arundinacea","PLDID"  = "Platanthera dilatata",
            "Poaceae_misc" = "Poaceae sp.","POPA2" = "Poa palustris","POPA23"  = "Potentilla anserina-pacifica","POTR2" = "Poa trivialis",
            "RUCO2" = "Rumex conglomeratus", "RUOC3" = "Rumex occidentalis","SALA2"  = "Sagittaria latifolia","SALUL" = "Salix lasiandra",
            "SASC"  = "Salix scouleriana","SCTA2" = "Schoenoplectus tabernaemontani","SIHE4" = "Sidalcia hendersonii","SISU2" = "Sium suave",
            "SYSU4"  = "Symphyotrichum subspicatum","TRWO" = "Trifolium wormskjoldii","TYLA"  = "Typha latifolia","Asteraceae_misc" = "Asteracea sp.",
            "Carex_sp1" = "Carex sp.","CIAR4" = "Cirsium arvense","DUAR3"  = "Dulichium arundinaceum","Festuca_sp" = "Festuca sp.", 
            "HOBR2" = "Hordeum brachyantherum","HYFO7"  = "Hypericum formosum","JUEF" = "Juncus effusus","MIGU"  = "Mimulus guttatus",
            "MYGA"  = "Myrica gale","POHY"  = "Polygonum hydropiper","Salix_sp"   = "Salix sp.","SASI2" = "Salix sitchensis",
            "GAPA3" = "Galium palustre","PUPA3" = "Puccinella pauciflora","SCMI2" =   "Scirpus microcarpus"))
...
endemic <- join_40 %>% 
  mutate(endemic = ifelse(Species %in% c("Asteraceae_misc", "Carex_sp1", "Carex_sp2", "Festuca_sp", "Galium_sp", "Poaceae_misc", "Salix_sp"), "unknown",
                   ifelse(Species %in% c("AGST2", "CIAR4", "FEAR3", "IMCA", "IRPS", "LYEU", "LYSA2", "MEAQ", "MEAR4", "MYSC", "PHAR3", "POTR2", "POHY", "RUCO2", "SOAR2"), "exotic", "native"))) %>% 
  left_join(join_40) %>% 
...
library(ggpubr)
ggarrange(rich_bar, cover_bar, ncol = 1, nrow = 2, common.legend = TRUE, legend="bottom")
...
#Note: turnover requires a column for abundance (not species presence/absence), here, Cover_class. "Cover_class" is a course abundance value where 0 = absent, 1 = 1-25% cover, 2 = 25-50% cover, etc. Because I want to understand turnover within the assemblage, I average all cover for a given species within that assemblage (across all quadrats (plots))

## Note: all calculations are comparison of 1979-1999, and 1999-2019. 


# test.turnover <- join_40 %>% 
#   filter(!Year == 1979)
# 
# test.total <- turnover(df = test.turnover, 
#                      time.var = "Year", 
#                      species.var = "Species", 
...
# METR, MEAQ, LYSA/SIHE, PHAR/IRPS
freq.bogbean <- join_40 %>% 
   filter(Assemblage == "Bogbean") %>% 
   filter(Species %in% c("METR3", "MEAQ", "SIHE4", "LYSA2", "PHAR3", "IRPS")) %>% 
   mutate(Species = factor(Species, 
                           levels = c("METR3", "MEAQ", "SIHE4", "LYSA2", "PHAR3", "IRPS"), 
                           labels = c("METR3", "MEAQ", "SIHE4", "LYSA2", "PHAR3", "IRPS"))) %>% 
   mutate(Species = recode(Species, "METR3" = "Menyanthes trifoliata", "MEAQ" = "Mentha aquatica*", "LYSA2" = "Lythrum salicaria*", "SIHE4" = "Sidalcia hendersonii", "PHAR3" = "Phalaris arundinaceae*", "IRPS" = "Iris pseudocorus*"))

# FEAR, SALUL (tip for Disc), LYSA/SIHE, PHAR/IRPS 
freq.fescue <- join_40 %>% 
   filter(Assemblage == "Fescue") %>% 
   filter(Species %in% c("FEAR3", "SALUL", "SIHE4", "LYSA2", "PHAR3", "IRPS")) %>% 
   mutate(Species = factor(Species, 
                           levels = c("FEAR3", "SALUL", "SIHE4", "LYSA2", "PHAR3", "IRPS"), 
                           labels = c("FEAR3", "SALUL", "SIHE4", "LYSA2", "PHAR3", "IRPS"))) %>% 
  mutate(Species = recode(Species, "FEAR3" = "Festuca arundinacea*", "SALUL" = "Salix lucida",  "SIHE4" = "Sidalcia hendersonii", "LYSA2" = "Lythrum salicaria*", "PHAR3" = "Phalaris arundinaceae*", "IRPS" = "Iris pseudocorus*")) 

#CALY, SALUL, LYTHE/SIHE, PHAR/IRPS
freq.sedge <- join_40 %>% 
   filter(Assemblage == "Sedge") %>% 
   filter(Species %in% c("CALY3", "SALUL", "SIHE4", "LYSA2", "PHAR3",  "IRPS")) %>% 
   mutate(Species = factor(Species, 
                           levels = c("CALY3", "SALUL", "SIHE4", "LYSA2", "PHAR3",  "IRPS"), 
                           labels = c("CALY3", "SALUL", "SIHE4", "LYSA2", "PHAR3",  "IRPS"))) %>% 
   mutate(Species = recode(Species, "CALY3" = "Carex lyngbyei", "SALUL" = "Salix lucida",  "SIHE4" = "Sidalcia hendersonii", "LYSA2" = "Lythrum salicaria*", "PHAR3" = "Phalaris arundinaceae*", "IRPS" = "Iris pseudocorus*"))
